...
public class SemanticHighlightingReferenceRequestor extends SemanticReferenceRequestor {
    ...
    public VisitStatus acceptASTNode(ASTNode node, TypeLookupResult result, IJavaElement enclosingElement) {
        if (node instanceof ImportNode) {
            if (node.getNodeMetaData("module.offset") instanceof Integer offset && offset > 0) { // GROOVY-11916
                typedPositions.add(new HighlightedTypedPosition(offset, "module".length(), HighlightKind.KEYWORD));
            }
